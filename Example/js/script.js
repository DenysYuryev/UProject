...
function isOpen(prop) {
    let answer = '';
    prop ? answer = 'Відкрито' : answer = 'Закрито'; // Замінити місцями: Закрито/Відкрито

    return answer; // НЕ ПРАВИЛЬНО: anwser
}

console.log(isOpen(restorantData.openNow)); // НЕ ПРАВИЛЬНО: console.log(isOpen(openNow))

// 
function isAverageLunchPriceTrue(fDish, sDish, average) {
    if (+fDish.price.slice(0, -1) + (+sDish.price.slice(0, -1)) < +average.slice(0, -1)) {  // НЕ відділено знак $ в ціні:  (+fDish.price.slice(0, -1) + (sDish.price) < average)
